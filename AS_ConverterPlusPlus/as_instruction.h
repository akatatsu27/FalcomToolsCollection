...
struct instruction
{
	...
	size_t textBuffOffset;

    static instruction parse(context* const ctx, string* const text)
    {
		char buffer[500];
#define b ctx->u8()
#define s ctx->u16()
...
#define txt text->append(&buffer[0]);
        ...
        instr.opcode = ctx->u8();
		instr.textBuffOffset = text->length();
        switch(instr.opcode)
		{
		...
		case 0x0C:{
			...
			txt	break;}
		case 0x0D:{
			uint8 op1 = b; uint8 op2 = b; uint32 op3 = i; uint32 op4 = i; uint32 op5 = i; uint16 op6 = s; uint16 op7 = s;
		...
		case 0x10:{
			...
			txt	break;}
		case 0x11:{
			uint8 op1 = b; uint8 op2 = b; uint32 op3 = i; uint32 op4 = i; uint32 op5 = i; uint32 op6 = i; uint8 op7 = b;
			sprintf(&buffer[0], "move 0x%02X, 0x%02X, 0x%08X, 0x%08X, 0x%08X, 0x%08X, 0x%02X\n", op1, op2, op3, op4, op5, op6, op7);
			txt	break;}
		case 0x12:{
			uint16 op1 = s; string op2 = str;
			snprintf(&buffer[0], 500, "add_effect 0x%04X, \"%s\"\n", op1, op2.data());
			txt	break;}
		case 0x13:{
			uint16 op1 = s;
		...
		case 0x17:{
			...
			txt	break;}
		case 0x18:{
			uint8 op1 = b; uint8 op2 = b; uint8 op3 = b; uint16 op4 = s; uint32 op5 = i; uint32 op6 = i; uint32 op7 = i; uint16 op8 = s; uint16 op9 = s; uint16 op10 = s; uint32 op11 = s; uint16 op12 = s; uint32 op13 = s; uint8 op14 = b;
			...
			txt	break;}
		case 0x19:{
			uint8 op1 = b; uint8 op2 = b; string op3 = str; uint16 op4 = s; uint32 op5 = i; uint32 op6 = i; uint32 op7 = i; uint16 op8 = s; uint16 op9 = s; uint16 op10 = s; uint32 op11 = s; uint16 op12 = s; uint32 op13 = s; uint8 op14 = b;
		...
		case 0x21:{
			...
			txt	break;}
		case 0x22:{
			uint8 target = b; uint8 op2 = b; uint16 op3 = s; uint8 op4 = b;
			if(target == 0xFF)
				sprintf(&buffer[0], "begin_thread self, 0x%02X, 0x%04X, 0x%02X\n", op2, op3, op4);
			else
				sprintf(&buffer[0], "begin_thread 0x%02X, 0x%02X, 0x%04X, 0x%02X\n", target, op2, op3, op4);
			txt	break;}
		case 0x23:{
			uint8 op1 = b; uint8 op2 = b;
		...
		case 0x27:{
			...
			txt	break;}
		case 0x28:{
			uint8 op1 = b; string op2 = str; uint32 op3 = i;
		...
		case 0x2B: // no operands
			...
			break;
		case 0x2C:{
			uint8 op1 = b; uint16 op2 = s; uint16 op3 = s;
		...
		case 0x30:{
			uint8 op1 = b;
			snprintf(&buffer[0], 500, "as_30 0x%02X", op1);
			txt
			do
        	{
            	string string = str;
            	...
            	snprintf(&buffer[0], 500, ", \"%s\"", string.data());
				txt
        	} while (1);
			break;}
		case 0x31:{
		...
		case 0x4A:{
			...
			txt	break;}
		case 0x4B:{
			uint8 op1 = b; uint8 op2 = b; uint32 op3 = i; uint16 op4 = s;
			sprintf(&buffer[0], "random 0x%02X, 0x%02X, 0x%08X, 0x%04X\n", op1, op2, op3, op4);
			txt	break;}
		case 0x4C:{
			uint16 op1 = s;
			sprintf(&buffer[0], "loop_target_beg 0x%04X\n", op1);
			txt	break;}
		...
		case 0x4E: // no operands
			...
			break;
		case 0x4F:{
			uint8_t op1 = b; uint8_t op2 = b;
			sprintf(&buffer[0], "as_4f 0x%02X, 0x%02X\n", op1, op2);
			txt	break;}
		case 0x50:{
			uint16 op1 = s;
			sprintf(&buffer[0], "call 0x%04X\n", op1);
			txt	break;}
		...
		case 0x59:{
			uint8 op1 = b; uint16 op2 = s;
			snprintf(&buffer[0], 500, "as_59 0x%02X, 0x%04X\n", op1, op2);
			txt break;}
		case 0x5A:{
			uint8 op1 = b; uint8 op2 = b; uint32 op3 = i;
		...
		case 0x63:{
			uint8 op1 = b; uint32 op2 = i;
			snprintf(&buffer[0], 500, "as_63 0x%02X, 0x%08X\n", op1, op2);
			txt	break;}
		case 0x64:{
			uint16 op1 = s;
		...
		case 0x67:{
			string op1 = str;
			snprintf(&buffer[0], 500, "scraft_cut_in \"%s\"\n", op1.data());
			txt	break;}
		...
		case 0x70:{
			...
			snprintf(&buffer[0], 500, "as_70 0x%02X, 0x%02X, 0x%04X, 0x%04X\n", op1, op2, op3, op4);
			txt	break;}
		case 0x71:{
			uint8 op1 = b;
		...
		case 0x7D:{
			uint8 op1 = b; uint32 op2 = i;
			snprintf(&buffer[0], 500, "as_7d 0x%02X, 0x%08X\n", op1, op2);
			txt	break;}
		case 0x7E:{
			uint32 op1 = i;
		...
		case 0x84:{
			...
			snprintf(&buffer[0], 500, "rotate_char 0x%02X, 0x%04X, 0x%04X, 0x%04X, 0x%08X, 0x%02X\n", op1, op2, op3, op4, op5, op6);
			txt	break;}
		case 0x85:{
			uint8 op1 = b; uint8 op2 = b; uint32 op3 = i;
		...
		case 0x92:{
			...
			txt	break;}
		case 0x93:{
			uint8 op1 = b; uint8 op2 = b; string op3 = str;
		...
		case 0x95: // no operands
			...
			break;
		case 0x96:{
			uint8 op1 = b; string op2 = str; uint16 op3 = s;
		...
		case 0x9F:{
			...
			txt	break;}
		case 0xA0:{
			uint8 op1 = b; uint32 op2 = i; uint16 op3 = s; string op4 = str;
			snprintf(&buffer[0], 500, "as_a0 0x%02X, 0x%08X, 0x%04X, \"%s\"\n", op1, op2, op3, op4.data());
			txt	break;}
		case 0xA1:{
			uint8 op1 = b; uint32 op2 = i;
		...
		case 0xA4:{
			...
			else if (op1 == 0x00 ){text->append("as_a400\n");}
			else if (op1 == 0x01) {text->append("as_a401\n");}
			else
			{
				instr.opcode = 0xFF;
				printf("tried to read invalid opcode %X%X at offset %X\n", instr.opcode, op1, instr.offset);
			}
			break;}
		case 0xA5:{
		...
		default:
			printf(text->data());
            printf("%ls tried to read invalid opcode %X at offset 0x%X\n", ctx->filename.c_str(), instr.opcode, instr.offset);
            instr.opcode = 0xFF;
		}
        return instr;
#undef b
