...
namespace BTSET1_Converter
{
    internal class LeJson
    {
        ...
        //The offsets in the BattleEntryOffsets Table. NOT THE OFFSETS OF THE ENTRIES THEMSELVES
        [JsonIgnore] private Dictionary<string, ushort> BattleEntryOffsets_Battles1 = new();
        [JsonIgnore] private Dictionary<string, ushort> BattleEntryOffsets_AutoBattles = new();
        [JsonIgnore] private Dictionary<string, ushort> BattleEntryOffsets_Battles2 = new();

        private LeJson() { }
        public LeJson(BTSET1 file)
        {
            string name;
            string str = "Bonuses";
            uint count = 0;
        ...
        internal static async Task<LeJson> Deserialize(string dirPath)
        {
        ...
        private void CalculateOffsets()
        {
            ...
            foreach (var battle in Battles2)
            {
                ...
                BattleEntryOffsets_Battles1[name] = curOffset;
            ...
            foreach (var battle in Battles1)
            {
                string name = battle.Key;
                BattleEntry entry = battle.Value;
                BattleEntryOffsets_Battles2[name] = curOffset;
                entry.FieldOffset = NameToOffsetMap[entry.BattlefieldName];
                foreach (var variation in entry.Variations)
                {
                    if (variation == null) continue;
            ...
            foreach (var battle in AutoBattles)
            {
                ...
                BattleEntryOffsets_AutoBattles[name] = curOffset;
                BattleEntryOffsets_Battles1[name] = curOffset;
                entry.FieldOffset = NameToOffsetMap[entry.BattlefieldName];
#if DEBUG
                //For some reason these battles are not present in the offset table
